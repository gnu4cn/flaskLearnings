...
angular.module("bootstrapLightbox", ["ui.bootstrap", "languageService"]);
try {
...
angular.module("bootstrapLightbox").run([
        ...
        a.put(
                ...
                <div class=lightbox-nav>\n\
                    ...
                    <div class=btn-group ng-if="Lightbox.images.length > 1">\n\
                        <a class="btn btn-xs btn-default" ng-click=Lightbox.prevImage()>\n\
                            ‹ {{lang.Previous}}</a>\n\
                        <a ng-href={{Lightbox.imageUrl}} target=_blank class="btn btn-xs btn-default" title="Open in new tab">\n\
                            {{lang.OpenImageinNewTab}}</a>\n\
            ...
            <div class=lightbox-image-container>\n\
                <div class=lightbox-image-caption>\n\
                    <span>{{Lightbox.imageCaption}}</span></div><img ng-if=!Lightbox.isVideo(Lightbox.image) lightbox-src={{Lightbox.imageUrl}}><div ng-if=Lightbox.isVideo(Lightbox.image) class="embed-responsive embed-responsive-16by9"><video ng-if=!Lightbox.isSharedVideo(Lightbox.image) lightbox-src={{Lightbox.imageUrl}} controls autoplay></video><embed-video ng-if=Lightbox.isSharedVideo(Lightbox.image) lightbox-src={{Lightbox.imageUrl}} ng-href={{Lightbox.imageUrl}} iframe-id=lightbox-video class=embed-responsive-item><a ng-href={{Lightbox.imageUrl}}>Watch video</a></embed-video></div></div></div>');
    }
]),
    ...
    this.templateUrl = "lightbox.html",
        ...
        this.calculateImageDimensionLimits = function (a) {
            return a.windowWidth >= 768 ? {maxWidth: a.windowWidth - 92, maxHeight: a.windowHeight - 126} : {maxWidth: a.windowWidth - 52, maxHeight: a.windowHeight - 86};
        },
        this.calculateModalDimensions = function (a) {
            var b = Math.max(400, a.imageDisplayWidth + 32), c = Math.max(200, a.imageDisplayHeight + 66);
            return(b >= a.windowWidth - 20 || a.windowWidth < 768) && (b = "auto"), c >= a.windowHeight && (c = "auto"), {width: b, height: c};
        },
                            ...
                            angular.extend({
                                ...
                                controller: ["$scope", 'languageService',
                                    function (a) {
                                        ...
                                        // 这里的a就是$scope?
                                        a.lang = JSON.parse(languageService());
                                        //console.log(JSON.stringify($scope.lang));
                                    }],
                                windowClass: "lightbox-modal"
                            },
                            d || {})),
                            g.modalInstance.result["finally"](
                            function () {
    ...
    angular.module("bootstrapLightbox").directive("lightboxSrc", ["$window", "ImageLoader", "Lightbox", function (a, b, c) {
        var d = function (a, b) {
            ...
            if (b) {
                ...
                e > c && f > d ? c / d > g / h ? (j = f, i = Math.round(c * f / d)) : (i = e, j = Math.round(d * e / c)) : e > c ? (i = e, j = Math.round(d * e / c)) : f > d && (j = f, i = Math.round(c * f / d)), c > g && d > h ? c / d > g / h ? (i = g, j = Math.round(d * g / c)) : (j = h, i = Math.round(c * h / d)) : c > g ? (i = g, j = Math.round(d * g / c)) : d > h && (j = h, i = Math.round(c * h / d));
        ...
        return{link: function (h, i, j) {
                var k = function () {
                    var b = a.innerWidth, h = a.innerHeight, j = c.calculateImageDimensionLimits({windowWidth: b, windowHeight: h, imageWidth: f, imageHeight: g}), k = d(angular.extend({width: f, height: g, minWidth: 1, minHeight: 1, maxWidth: 3e3, maxHeight: 3e3}, j), c.fullScreenMode), l = c.calculateModalDimensions({windowWidth: b, windowHeight: h, imageDisplayWidth: k.width, imageDisplayHeight: k.height});
                    i.css({width: k.width + "px", height: k.height + "px"}), angular.element(document.querySelector(".lightbox-modal .modal-dialog")).css({width: e(l.width)}), angular.element(document.querySelector(".lightbox-modal .modal-content")).css({height: e(l.height)});
                };
                h.$watch(function () {
