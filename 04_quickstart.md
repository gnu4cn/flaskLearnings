...
请注意这些cookies是设置在response对象上的。因为通常地返回自那些视图函数的都仅是字串，所以Flask都会将它们转换成response对象。如要显式地完成这一转换，就可使用`make_resonse()`函数，然后对其进行修改。
...
来自某个视图函数的返回值，被自动转换成一个response对象。而如过返回值是一个字符串，则就将该字符串作为响应主体，再加上一个`200 OK`的状态码及一个*text/html*的mimetype，而转换成响应对象。在将返回值转换成响应对象时，Flask用到的逻辑如下：
...
有时可能会遇到所处理的数据应该是正确的，但实际则不然的情形。比如可能得到了一些发出一个HTTP请求到服务器的客户端代码，但却是明显格式错误的代码。这可能是由于用户对数据做了篡改，或者客户端代码失误造成的。在那种情况下，多数时候回应其一个`400 Bad Request`就可以了，但有时却没有作用，代码会继续运行。
